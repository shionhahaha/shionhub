...
local ServerSortMode = "Ping" -- デフォルトのソート設定
...
ServerTab:CreateSection("サーバーブラウザ設定")

ServerTab:CreateDropdown({
   Name = "並び替え順序を選択",
   Options = {"Ping (低い順)", "人数 (多い順)", "人数 (少ない順)"},
   CurrentOption = "Ping (低い順)",
   Callback = function(Option)
      local choice = type(Option) == "table" and Option[1] or Option
      if choice == "Ping (低い順)" then ServerSortMode = "Ping"
      elseif choice == "人数 (多い順)" then ServerSortMode = "Most"
      elseif choice == "人数 (少ない順)" then ServerSortMode = "Least"
      end
   end,
})
...
ServerTab:CreateButton({
   ...
   Callback = function()
      ...
      if Data and Data.data then
         ...
         -- 並び替えロジック
         if ServerSortMode == "Ping" then
            table.sort(tempTable, function(a, b) return (a.ping or 999) < (b.ping or 999) end)
         elseif ServerSortMode == "Most" then
            table.sort(tempTable, function(a, b) return (a.playing or 0) > (b.playing or 0) end)
         elseif ServerSortMode == "Least" then
            table.sort(tempTable, function(a, b) return (a.playing or 0) < (b.playing or 0) end)
         end
         ...
         Rayfield:Notify({Title = "リスト更新", Content = "ソート完了: " .. ServerSortMode})
      end
   end,
})
