...
-- --- サーバー設定タブ ---
ServerTab:CreateSection("現在のサーバー情報")

ServerTab:CreateButton({
   Name = "現在のサーバーIDをコピー",
   Callback = function()
      setclipboard(tostring(game.JobId))
      Rayfield:Notify({Title = "コピー完了", Content = "JobIdをクリップボードにコピーしました"})
   end,
})

ServerTab:CreateButton({
   Name = "サーバーを再起動 (Rejoin)",
   Callback = function()
      game:GetService("TeleportService"):TeleportToPlaceInstance(game.PlaceId, game.JobId, game.Players.LocalPlayer)
   end,
})

ServerTab:CreateSection("手動ID入力で移動")

ServerTab:CreateInput({
   Name = "移動先サーバーIDを入力",
   PlaceholderText = "JobIdをここに貼り付け",
...
ServerTab:CreateButton({
   ...
   Callback = function()
      if TargetServerID ~= "" then
         Rayfield:Notify({Title = "移動開始", Content = "指定されたIDへテレポートします"})
         game:GetService("TeleportService"):TeleportToPlaceInstance(game.PlaceId, TargetServerID, game.Players.LocalPlayer)
      else
         Rayfield:Notify({Title = "エラー", Content = "IDを入力してください"})
      end
   end,
})
...
local ServerDropdown = ServerTab:CreateDropdown({
   ...
   Callback = function(Option)
      ...
      if ServerListData[label] then
         SelectedServerID = ServerListData[label]
      end
   end,
})
   ...
   Callback = function()
      ...
      if Data and Data.data then
         ...
         for _, s in pairs(tempTable) do
            local currentPlayers = s.playing or 0
            local maxPlayers = s.maxPlayers or 0
            local playersLabel = "[" .. currentPlayers .. "/" .. maxPlayers .. "]"
            local ping = s.ping or "???"
            local label = ""
            
            if type(ping) == "number" and ping < 120 then
               label = "✅[Asia推定] " .. playersLabel .. " Ping: " .. ping
            else
               label = "🌐[Other] " .. playersLabel .. " Ping: " .. ping
            end
...
ServerTab:CreateButton({
   ...
   Callback = function()
      if SelectedServerID ~= "" then
         ...
         Rayfield:Notify({Title = "エラー", Content = "リストからサーバーを選択してください"})
      end
   end,
})
