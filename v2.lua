...
local Window = Rayfield:CreateWindow({
   Name = "カスタムツール V8.0 (Server Hop Integrated)",
...
-- --- サーバー設定タブ (新規追加) ---
ServerTab:CreateSection("サーバーホッパー・ID移動")
...
ServerTab:CreateButton({
   ...
   Callback = function()
      if TargetServerID ~= "" then
         Rayfield:Notify({Title = "テレポート", Content = "指定されたサーバーへ移動を開始します..."})
         game:GetService("TeleportService"):TeleportToPlaceInstance(game.PlaceId, TargetServerID, game.Players.LocalPlayer)
      else
         Rayfield:Notify({Title = "エラー", Content = "サーバーIDを入力してください"})
      end
   end,
})

ServerTab:CreateButton({
   Name = "ランダムなサーバーへ移動 (Server Hop)",
   Callback = function()
      ...
      if Target.id == game.JobId then
         Rayfield:Notify({Title = "ホップ", Content = "新しいサーバーを探しています..."})
         Target = Servers.data[math.random(1, #Servers.data)]
      end
      
      TPS:TeleportToPlaceInstance(game.PlaceId, Target.id, game.Players.LocalPlayer)
   end,
})
